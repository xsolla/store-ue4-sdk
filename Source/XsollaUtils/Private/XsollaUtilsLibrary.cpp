...
bool UXsollaUtilsLibrary::ParseTokenPayload(const FString& Token, TSharedPtr<FJsonObject>& PayloadJsonObject)
{
...
FString UXsollaUtilsLibrary::GetTokenParameter(const FString& Token, const FString& Parameter)
{
	TSharedPtr<FJsonObject> PayloadJsonObject;
	if (!ParseTokenPayload(Token, PayloadJsonObject))
	{
		UE_LOG(LogXsollaUtils, Log, TEXT("%s: Can't parse token payload"), *VA_FUNC_LINE);
		return FString();
	}
	...
	{
		UE_LOG(LogXsollaUtils, Warning, TEXT("%s: Can't find parameter %s in token payload"), *VA_FUNC_LINE, *Parameter);
		return FString();
	}
